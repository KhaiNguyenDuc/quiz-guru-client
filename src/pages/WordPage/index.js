...
const WordPage = () => {
  ...
  const findWordsByWordSet = async (page) => {
    ...
    if (response?.status !== 400) {
      const listWords = response?.data?.words.map(
      ...
      {isLoading ? (
        ...
        <div>
          ...
          {words?.map((word, index) => {
            ...
            if (word?.definition?.entries[0] !== undefined || word?.content !== null || word?.name !== null ) {
              return <Word setWords={setWords} word={word?.definition} id={word?.id} content={word?.content} key={index} name={word?.name}/>;
            }
            return <></>
